...
import AlertNotifyPopup from "../AlertNotifyPopup/alertNotifyPopup";
      ...
      <div className="schedule-contact-modal bg-white p-6 rounded-2xl shadow-lg z-20 h-auto">
        ...
        <div className="text-center">
          ...
          <p className="text-sm font-normal text-gray-500 my-5">
            {selectedOption === "Call"
              ? "Call the customer to inform/ ask about the schedule"
              : selectedOption === "Whatsapp"
              ? "Send a text via WhatsApp"
              : selectedOption === "Gmail"
              ? "Send a mail to inform the customer"
          ...
          {selectedOption ? (
            ...
            <div
              className={`flex items-center rounded-full p-3 ${optionColors[selectedOption]}`}
            >
              {selectedOption === "Call" && (
                <img
                  src={Call}
                  className="w-6 h-6 mr-2 border-e pe-2 border-[#94D82D]"
                  alt="Call"
                />
              )}
              {selectedOption === "Whatsapp" && (
                <img
                  src={Whatsapp}
                  className="w-6 h-6 mr-2 border-e pe-2 border-[--whatsapp-green-color]"
                  alt="Whatsapp"
                />
              )}
              {selectedOption === "Gmail" && (
                <img
                  src={Gmail}
                  className="w-6 h-5 mr-2 border-e pe-2 border-[--blue-color]"
                  alt="Gmail"
                />
              )}
              ...
              <button type="submit">
                ...
                {selectedOption === "Whatsapp" && (
                  <p className="text-whatspp-green-color text-base font-medium mr-2">
                    Send
                  </p>
                )}
                {selectedOption === "Gmail" && (
                  <p className="text-blue-color text-base font-medium mr-2">
                    Send
                  </p>
                )}
              </button>
